...
**What gets installed:** `scripts/brain_daily.py` runs headless daily (cheap model, read-only tools + `query.sh` for dedup), driven by `prompts/distill_daily.md`. It collects the owner's AI sessions ONLY from the opted-in folders (interview step 3); filters (minimum size, minimum owner messages, exclude its own headless runs); **sanitizes before any LLM call via `scripts/sanitize.py`** (strips API keys/tokens/passwords, email addresses, phone numbers, and the owner's declared confidential patterns; this step is code, not a prompt, so it cannot be argued with by the model); distills at most 5 candidates/day, all candidate content in the owner's MAIN language, into `{type, title, body, evidence, proposed_destination, nature, domain, reason_not_to_enter}` (`nature`/`domain`: the heuristics-clustering axes from `scripts/heuristics_taxonomy.py`, validated and possibly corrected downstream by the judge); writes each as `status: draft` into the gate queue; rebuilds the queue index; notifies the phone via `scripts/notify.py` (system notification, English; see the communication contract exception).
...
Details that broke in test installs: notification counts must be unambiguous (the shipped scripts use the "N candidate(s)" form); `proposed_destination` must be a canonical relative path (parseable by code), with any commentary in a separate field.
...
**Part A (telemetry and THE eligibility rule, defined once):** `scripts/gate_log.py`: append-only JSONL decision log + policy file in `_system/telemetry/`; commands `add` (file, type, decision approved|edited|discarded, destination, note) and `stats`. **Autonomy eligibility rule (the ONLY definition; phases 1 and 4 refer here): a candidate type becomes eligible when n>=20 logged decisions, >=95% approved-unchanged, across >=6 distinct weeks.** Honor the `BRAIN_CONFIG` env override (point it at a sandbox config.json) for sandbox tests.

**Part B (weekly ritual):** `scripts/brain_weekly.py` and `scripts/brain_weekly_auto.py`: deterministic lint + cosmetic-only auto-fixes in a separate revertible commit + numbered dispatch package (candidates + recommendations, drafted with `prompts/weekly_dispatch.md`) + telemetry section + phone notification (English, like all system notifications); report and dispatch content in the owner's language. Owner answers one message; the dispatching session applies, deletes drafts, and MUST log every decision via `gate_log.py`. Install the `templates/commands/dispatch-weekly.md` command so any session knows the procedure.

**Part C (watchdog):** `scripts/brain_watchdog.py`, every 6h + at boot: alert (high priority) if daily hasn't run >36h, weekly missing, retrieval daemon down (restart first), queue >15 or oldest >30 days, index stale >12h (config `thresholds.index_stale_h`; self-heal reindex), safety copy behind (self-heal push, alert on failure). 12h anti-spam per alert key.
...
**What gets installed:** `scripts/gate_judge.py` driven by `prompts/judge.md`, a strong-model job judging each candidate: `approve | edit | discard | escalate`; mandatory semantic dedup; structured output with polished final text (owner's language), 2-4 validated `related` links (NFC), optional destination redirect (complement an existing note instead of creating an isolated one). Deterministic appliers write with `approved_by` provenance. Every approved or edited candidate is also classified along the `nature`/`domain` axes (heuristics clustering, criterion 10 in `prompts/judge.md`), validated against the closed vocabulary in code before the class label is written; when a rule should act as a skill, quality gate or governance rule but does not yet, the judge may propose a one-line `promotion`, recorded in the owner's routing note (`taxonomy.heuristics.routing`, optional) for them to act on, never applied automatically. An optional `judge_model_heuristics` config key lets lesson/heuristic candidates be judged with a different model than the rest of the queue. STRUCTURAL escalation: facts about people's roles/identity, contradiction with active notes, employer/client-identifiable material, unknown destinations, past the cap (max 10 applications/run), all delete/merge/supersede. Bias: in doubt, discard or escalate.
...
**Spec:** `scripts/brain_aging.py`, a monthly aging pass (sample up to 15 dynamic notes unchecked 90+ days vs the owner's 00-HOME ground truth; auto-mark `stale` reversibly; escalate direct contradictions; state file in telemetry) + the connection audit from phase 1.5, re-run quarterly for new islands as a MANUAL ritual: no service template ships for it (it is a prompt, not a script). Tell the owner, in the final guide, to ask their assistant to re-run the audit prompt roughly once a quarter, e.g. when the monthly aging report arrives.
