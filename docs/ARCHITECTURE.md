...
(eligibility rule met + backtest + owner's yes). `judge_model_heuristics` is
optional (default ""): when set, `gate_judge.py` judges lesson/heuristic
candidates with that model instead of the base `--model`, falling back to the
base model if the alternate one fails; leave it empty to judge every
candidate with the base model, as in v1.8. `taxonomy.heuristics.routing` is
optional: the relative path of a note the owner keeps for promotion suggestions
(see "Heuristics clustering" below); left empty (the default), a promotion
suggestion is only logged, never written anywhere.

## Heuristics clustering (two-axis classification)

Every lesson, pattern and decision the gate approves or edits is classified
along two closed axes, defined once in `scripts/heuristics_taxonomy.py`:

- **nature**: WHERE the rule should act (`decision-tree`, `score`, `judgment`,
  `axiom`, or the honest fifth value `one-off-decision` for a dated,
  non-reusable project choice). Each nature maps deterministically to a route
  (skill, quality gate, agent-brain, governance, or record); the model never
  decides the route, only the nature.
- **domain**: the TYPE of decision the rule guides (verification-qa,
  deploy-delivery, architecture-code, ai-orchestration, data-facts,
  communication-copy, security-privacy, git-versioning, process-workflow,
  business-strategy).

`prompts/distill_daily.md` asks the cheap model to propose both axes;
`prompts/judge.md` criterion 10 makes classification mandatory for approve or
edit, validated in code against the closed vocabulary (an invalid value
becomes empty, it is never invented) before `scripts/gate_judge.py`'s
appliers write the label. When a rule's nature is `decision-tree`, `score` or
`axiom` and it does not yet act on a real surface, the judge may propose a
`promotion`; this is a suggestion recorded in the owner's routing note
(`taxonomy.heuristics.routing`, if configured) for the owner to act on, never
applied automatically. `scripts/classify_heuristics.py` is the re-runnable
backfill and route audit for whatever slipped through without a class before
this feature existed, or was added outside the daily pipeline.
...
scripts/                  brain_config.py · embed_brain.py · brain_daemon.py · query_brain.py
                          ...
                          heuristics_taxonomy.py · classify_heuristics.py
...
tests/                    test_sanitizer_corpus.py · test_commit_scoped.py · judge fixtures (6) ·
                          aging fixtures (2) · test_config.py · test_heuristics_taxonomy.py ·
                          test_heuristics_classification.py · run_all.sh
